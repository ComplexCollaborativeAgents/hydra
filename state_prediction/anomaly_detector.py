...
class FocusedSBAnomalyDetector(FocusedAnomalyDetector):
    def __init__(self, threshold=0.5):
        ...
        self.state_predictor.load_state_dict(
            torch.load('{}/state_prediction/fc_single_channel_posweight/pretrained_model_fc.pt'.format(settings.ROOT_PATH),
                       map_location=self.device))
        with open('{}/state_prediction/fc_single_channel_posweight/fc_pretrained_novelty_detector.pickle'.format(
                settings.ROOT_PATH), 'rb') as f:
