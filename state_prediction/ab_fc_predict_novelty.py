...
no_novelty_set = ABDataset('data_2021416_preproc', mode='test')
...
cnn = MyCNN(device)
cnn.load_state_dict(torch.load('fc_single_channel_posweight/pretrained_model_fc.pt', map_location=device))
cnn.eval()
...
with torch.no_grad():
    ...
    for states, actions, next_states in no_noveltyloader_test:
        ...
        states = states.to(device, non_blocking=True)
        next_states = next_states.to(device, non_blocking=True)
        states_resized = (states.sum(axis=1).reshape(next_states.shape[0], -1) > 0).type(torch.float32)
        next_states_resized = (next_states.sum(axis=1).reshape(next_states.shape[0], -1) > 0).type(torch.float32)
        y_hat = cnn(states_resized, actions)
...
for train, test in train_test_pairs:
    novelty_train_set = torch.utils.data.ConcatDataset([ABDataset('data_2021416_preproc', mode=level) for level in train])
    ...
    novelty_test_set = torch.utils.data.ConcatDataset([ABDataset('data_2021416_preproc', mode=level) for level in test])
...
novelty_set = torch.utils.data.ConcatDataset([ABDataset('data_2021416_preproc', mode=level) for level in all_novelties])
