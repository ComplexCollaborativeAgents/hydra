...
class ABDataset(TensorDataset):
    ...
    def _load_tensors(self):
        ...
        if not os.path.isfile(os.path.join(self.path, self.mode + '_data.npz')):
            num_obs = len(glob.glob1(os.path.join(self.path, self.mode),
                                     '*.npz'))  # https://stackoverflow.com/questions/1320731/count-number-of-files-with-certain-extension-in-python
            drop_idx = []
            print('one by one')
            ...
            np.savez_compressed(os.path.join(self.path, self.mode + '_data.npz'), state=all_state, action=all_action,
...
if __name__ == '__main__':
    ...
    torch_dataset = ABDataset('data_test_train_preproc', transform_s, transform_a, mode='novelty_new')
