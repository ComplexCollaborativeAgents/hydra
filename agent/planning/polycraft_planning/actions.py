...
class TeleportToAndDo(MacroAction):
    ...
    def _action_at_cell(self, state: PolycraftState):
        raise NotImplementedError(
            "Subclasses should implement this. Returns the next action to do after facing a cell. Do not forget to mark _is_done when done.")
...
class ExploreRoom(PolycraftAction):
    ...
    def do(self, state: PolycraftState, env: Polycraft):
        ...
        for cell_id, cell_attr in sensed_game_map.items():
            ...
            env.door_to_room_cells[self.door_cell][
                cell_id] = cell_attr  # A cell may exists in multiple door-to-cells entries
...
class TeleportToTraderAndTrade(TeleportToAndDo):
    ...
    def __init__(self, trader_id, trade):
        super().__init__(cell=None,
                         max_steps=TeleportAndFaceCell.MAX_STEPS + 1)  # Cell is determined in runtime, according to the location of the trader
...
class CreateByRecipe(MacroAction):
    ...
    def __init__(self, item_to_craft: str,
        ...
        super().__init__(
            max_steps=20)  # TODO: 20 here is a magic number. Sine this action is not used by our main agent, it's good enough for now.
...
class CreateBlockOfPlatinum(CreateByRecipe):
    ...
    def __init__(self):
        super().__init__(item_type_to_get=BlockType.BLOCK_OF_PLATINUM.value,
