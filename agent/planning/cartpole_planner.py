...
class CartPolePlanner(HydraPlanner):
    ...
    def execute(self, plan, policy_learner):
        ...
        assert False  #TODO (Yoni) then why does this function exist??
    ...
    def write_problem_file(self, pddl_problem):
        ...
        if settings.DEBUG:
            ...
            exporter.to_file(pddl_problem,
                             "{}/trace/problems/{}_cartpole_problem.pddl".format(settings.CARTPOLE_PLANNING_DOCKER_PATH,
    ...
    def get_plan_actions(self, count=0):
        nyx.runner("%s/cartpole_domain.pddl" % str(settings.CARTPOLE_PLANNING_DOCKER_PATH),
                   ...
                   ['-vv', '-to:%s' % str(settings.CP_TIMEOUT), '-noplan', '-search:gbfs', '-custom_heuristic:4',
