...
def heuristic_function(state):
    ...
    if constants.CUSTOM_HEURISTIC_ID == 1:
        ...
        if not bird_released:
            ...
            targets_x = {obj[9:]: state.state_vars[obj] for obj in state.state_vars.keys() if
                         obj.startswith("['x_block")}
            targets_x.update(
                {obj[7:]: state.state_vars[obj] for obj in state.state_vars.keys() if obj.startswith("['x_pig")})
            targets_w = {obj[13:]: state.state_vars[obj] for obj in state.state_vars.keys() if
                         obj.startswith("['block_width")}
            pig_radii = {obj[12:]: state.state_vars[obj] for obj in state.state_vars.keys() if
            ...
            targets_y = {obj[9:]: state.state_vars[obj] for obj in state.state_vars.keys() if
                         obj.startswith("['y_block")}
            targets_y.update(
                {obj[7:]: state.state_vars[obj] for obj in state.state_vars.keys() if obj.startswith("['y_pig")})
            targets_h = {obj[14:]: state.state_vars[obj] for obj in state.state_vars.keys() if
                         obj.startswith("['block_height")}
