...
STATS_FILE_NAME = pathlib.Path(__file__).parent.absolute() / "consistency-results-all.csv"
...
def run_consistency_stats(consistency_checker_types: List[ConsistencyCheckerType],
                          ...
                          results_file: pathlib.Path = STATS_FILE_NAME):

    if seed is not None:
        random.seed(seed)

    outfile = results_file.open("w")
...
def analyze_results(results_file: pathlib.Path = STATS_FILE_NAME):
...
if __name__ == '__main__':
    run_consistency_stats([ConsistencyCheckerType.ScienceBirds,
                          ConsistencyCheckerType.BirdLocation,
                          ConsistencyCheckerType.BlockNotDead], seed=0)
    analyze_results()
