...
NON_NOVELTY_PERFORMANCE = 'non-novelty-performance'
UNKNOWN_NOVELTY = 'unknown-novelty'
KNOWN_NOVELTY = 'known-novelty'
    ...
    def __run_trial(self, episodes: range=(0,1), steps: int = 200):
        ...
        for episode in episodes:
            ...
            if self._results is not None:
                self._log_data(episode_num=episode, novelty_probability=novelty_probability, novelty_threshold=novelty_threshold, novelty=novelty, novelty_characterization=novelty_characterization, performance=sum(rewards))
        env.close()
        return self._results

    def _log_data(self, episode_num=0, novelty_probability=0, novelty_threshold=0, novelty=None, novelty_characterization=0, performance=0):
...
class NoveltyExperimentRunnerCartpole:
    def __init__(self,
        ...
        self._results_dataframe = pandas.DataFrame(columns=['trial_num', 'episode_num', 'type', 'novelty_probability', 'novelty_threshold', 'novelty', 'novelty_characterization', 'performance'])
        self._results_file = open(path.join(settings.ROOT_PATH, "data", "cartpole", "test", "repairing_test_wsu.csv"), "w")
    ...
    def run_non_novelty_performance_subtrial(self, episode_range, trial_num=0):
        ...
        results['type'] = NON_NOVELTY_PERFORMANCE
        self._results_dataframe = self._results_dataframe.append(results)

    def run_unknown_novelty_subtrial(self, episode_range, trial_num=0, novelty={'gravity': 10}):
        observer = CartpoleHydraAgentObserver(agent_type=RepairingCartpoleHydraAgent)
        env_dispatcher = NoveltyExperimentGymCartpoleDispatcher(observer, render=True)
        env_dispatcher.set_novelty(novelty)
        env_dispatcher.set_is_known(False)
        results = env_dispatcher.run_trial(episode_range=episode_range)
        results['trial_num'] = trial_num
        results['type'] = UNKNOWN_NOVELTY
        #results['novelty_config'] = novelty
        self._results_dataframe = self._results_dataframe.append(results)

    def run_known_novelty_subtrial(self, episode_range, trial_num=0, novelty={'gravity': 10}):
        observer = CartpoleHydraAgentObserver(agent_type=RepairingCartpoleHydraAgent)
        env_dispatcher = NoveltyExperimentGymCartpoleDispatcher(observer, render=True)
        env_dispatcher.set_novelty(novelty)
    ...
    def run_experiment_trial(self):
        ...
        for trial in range(0, self._number_of_experiment_trials):
            episode_num = 0
            self.run_non_novelty_performance_subtrial(episode_range=range(episode_num, episode_num + self._non_novelty_performance_trial_length))
            episode_num = episode_num + self._non_novelty_performance_trial_length
            self.run_unknown_novelty_subtrial(episode_range=range(episode_num, episode_num + self._novelty_trial_length))
            self.run_known_novelty_subtrial(episode_range=range(episode_num, episode_num + self._novelty_trial_length))
        self._results_dataframe.to_csv(self._results_file)
